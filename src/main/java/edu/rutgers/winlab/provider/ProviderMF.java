...
public class ProviderMF {
    ...
    public ProviderMF(String mapping, int staticFileWaitTime, int dynamicGUID) throws JMFException, IOException {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(mapping))) {
            ...
            while ((line = reader.readLine()) != null) {
                lineID++;
                line = line.trim();
                final int tmpLineID = lineID;
                final String tmpline = line;

                if (line.startsWith("#")) {
                    LOG.log(Level.INFO, () -> String.format("Skipping comment line %d: %s", tmpLineID, tmpline));
                    continue;
                }
                int idx = line.indexOf(' ');
                if (idx == -1) {
                    LOG.log(Level.INFO, () -> String.format("Skipping error line (no space): %d: %s", tmpLineID, tmpline));
                    continue;
                }
                int guid;
                try {
                    ...
                    LOG.log(Level.INFO, () -> String.format("Skipping error line (cannot parse guid): %d: %s", tmpLineID, tmpline));
                    continue;
                }
                String fileName = line.substring(idx).trim();
                if (handlers.containsKey(guid)) {
                    LOG.log(Level.INFO, () -> String.format("Skipping error line (duplicate GUID): %d: %s", tmpLineID, tmpline));
                }
                LOG.log(Level.INFO, () -> String.format("Got line: %d: GUID:%d->File:%s", tmpLineID, guid, fileName));
    ...
    public static void main(String[] args) throws JMFException, IOException {
        if (args.length < 3) {
            System.out.printf("Usage: java %s <mapping> <wait> <dynamicGUID>%n", ProviderMF.class.getName());
            return;
        }
        String mapping = args[0];
        int wait = Integer.parseInt(args[1]);
        int guid = Integer.parseInt(args[2]);
        LOG.log(Level.INFO, () -> String.format("Starting MF Provider mapping file %s, static file wait time %d, dynamic guid %d%n", mapping, wait, guid));
